...
export function cellGeometry(tess, factor = 1) {
    ...
    if (s.type === 'prism') {
        const g = new THREE.CylinderGeometry(s.radius * factor, s.radius * factor, s.height * factor, s.sides, 1)
            .toNonIndexed();
        ...
        for (let i = 0; i < uv.count; i++) if (Math.abs(nrm.getY(i)) < 0.5) uv.setX(i, uv.getX(i) * s.sides);
        return g;
    }
