...
export function customLevel({w, d, h, mines, tess} = {}) {
    const width = clamp(Math.round(Number(w) || DEFAULT_LEVEL.w), CUSTOM_LIMITS.min, CUSTOM_LIMITS.max);
    ...
    const wanted = Number(mines) || Math.round((cells * DEFAULT_LEVEL.mines) / (DEFAULT_LEVEL.w * DEFAULT_LEVEL.d * DEFAULT_LEVEL.h));
