...
export class Grid {
    ...
    cellAt(px, py, pz) {
        const l = this.tess.locate(px - this.origin.x, py - this.origin.y, pz - this.origin.z, this._loc);
    ...
    neighbourCount() {
        ...
        return (face > 0 ? classes.face : 0) + (edge > 0 ? classes.edge : 0) + (corner > 0 ? classes.corner : 0);
    }
...
function buildNeighbourTable(grid) {
    ...
    if (sample.length > NEIGHBOUR_STRIDE) {
        throw new Error(`tessellation ${tess.id} has ${sample.length} neighbours, max ${NEIGHBOUR_STRIDE}`);
    }
