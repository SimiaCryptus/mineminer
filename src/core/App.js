...
export class App {
  ...
  startFromHash() {
    ...
    this.settings = {
      ...this.settings,
      edgeWeight: p.weights.edge,
      cornerWeight: p.weights.corner,
    };
  ...
  handleIntent(intent) {
    ...
    if (r.kind === 'noop') {
      ...
      else if (r.reason === 'marks-mismatch')
        this.hud.flash('Marks around this number do not match it');
    }
  ...
  onAction(r) {
    ...
    switch (r.kind) {
      ...
      case 'boom':
        ...
        if (!r.lost)
          this.hud.flash(
            `Detonation! ${this.board.lives} ${this.board.lives === 1 ? 'life' : 'lives'} left.`
          );
        break;
    }
    if (r.collapse) {
      this.sfx.collapse();
      const left = this.board.quantumLives;
      this.hud.flash(
        left === Infinity
          ? 'Ψ Superposition collapsed — the vault took your branch'
          : left > 0
            ? `Ψ Superposition collapsed — ${left} quantum ${left === 1 ? 'life' : 'lives'} left`
            : 'Ψ Last quantum life spent — the vault is classical from here'
      );
    }
    this.updateHud();
  ...
  updateHud() {
    ...
    this.hud.setQuantum(
      this.board.collapses,
      this.board.quantumLives,
      this.board.opts.quantum !== 'off'
    );
  }
