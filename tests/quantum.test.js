...
import { Grid, MINE, EMPTY, INTACT, MINED } from '../src/game/Grid.js';
import { placeMines } from '../src/game/MineGenerator.js';
import { Board } from '../src/game/Board.js';
import { frontierComponent, findWorld, classify, touchesNumber } from '../src/game/Solver.js';
...
function ringBoard(opts = {}) {
  ...
  const board = new Board(grid, { safeFirstStrike: false, cascade: 'off', seed: 'quantum', ...opts });
...
test('a provably safe cell cannot be crystallised: the mark stays plain and misfires', () => {
  ...
  grid.content[2] = MINE;
  grid.recomputeCounts();
  const board = new Board(grid, { safeFirstStrike: false, cascade: 'off', quantum: 'on', lives: 2 });
...
test('the safe first strike never moves a mine out from under a flag', () => {
   for (let s = 0; s < 20; s++) {
     ...
     for (const f of flags) assert.equal(grid.content[f], MINE, `${seed}: a blind flag pulls a mine under it`);
     ...
     for (const f of flags) assert.equal(grid.content[f], MINE, `${seed}: the flag still covers its mine`);
     assert.equal(totalMines(grid), 10, `${seed}: mine total conserved`);
     grid.forEachNeighbour(first, (n) => {
       if (!flags.includes(n)) assert.equal(grid.content[n], EMPTY, `${seed}: unflagged neighbour ${n} is clear`);
     });
...
test('deductions built on a flag stay true after the opening', () => {
   ...
   grid.forEachNeighbour(grid.index(1, 0, 1), (n) => {
     if (n !== flag) assert.equal(board.strike(n).kind, 'cleared', `neighbour ${n} of the 1 is safe`);
   });
...
test('replaying a log with collapses is exact', () => {
  const make = () => {
    ...
    grid.content[grid.index(0, 0, 0)] = MINE;
    grid.recomputeCounts();
    return new Board(grid, { safeFirstStrike: false, cascade: 'off', quantum: 'on', seed: 'replay-q' });
  };
  const a = make();
  ...
  for (let s = 0; s < 6; s++) {
    ...
    for (let step = 0; step < 80 && !board.isOver; step++) {
      ...
      const cell = intact[Math.floor(play() * intact.length)];
       ...
       assert.notEqual(verdict, 'unknown', `${seed} step ${step}: the frontier search ran out of budget`);
       ...
       if (r.kind === 'boom') {
         ...
         assert.ok(verdict === 'mine' || verdict === 'classical', `${seed} step ${step}: an ambiguous block detonated`);
       }
      assert.equal(totalMines(grid), 18, `${seed} step ${step}: mine total drifted`);
      for (const [i, c] of shown) assert.equal(grid.counts[i], c, `${seed} step ${step}: number ${i} changed`);
    }
  }
});