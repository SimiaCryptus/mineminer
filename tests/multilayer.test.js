...
import { Grid, INTACT, MINE, MINED } from '../src/game/Grid.js';
...
test('mine layouts are not repeated layer by layer', () => {
  ...
  const layer = (y) =>
    [...grid.content.slice(y * grid.layerSize, (y + 1) * grid.layerSize)].join('');
...
test('a cascade started on the top layer tunnels down through the slab', () => {
  ...
  assert.ok(
    r.revealed.some((x) => grid.y(x.cell) === 0 && x.delay > 0),
    'lower-layer reveals are delayed'
  );
